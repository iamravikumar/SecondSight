...
namespace SecondSight
{
    public class NewDBPage8 : InteriorWizardPage
    {
        ...
        protected internal override string OnWizardNext()
        {
            ...
            {
                if (Path.GetFileName(tb_NewDBPath.Text) == tb_NewDBPath.Text)
                { //If it's just a filename, add the default directory
                    tb_NewDBPath.Text = ndbv.DefaultPath + tb_NewDBPath.Text;
                }
                if (File.Exists(tb_NewDBPath.Text))
                {  //If the file already exists, prompt for overwrite.
