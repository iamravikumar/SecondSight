...
namespace SecondSight
{
    public partial class MainForm
    {
        ...
        private void tb_S_AddOD_Leave(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Convert.ToSingle(tb_S_AddOD.Text) != 0)
                { //If AddOD is a nonzero number
                    rb_S_Multi.Checked = true;
                }
                else if (Convert.ToSingle(tb_S_AddOS.Text) == 0)
                { //If AddOS (not a typo) is also zero
        ...
        private void tb_S_AddOS_Leave(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Convert.ToSingle(tb_S_AddOS.Text) != 0)
                { //If AddOS is a nonzero number
                    rb_S_Multi.Checked = true;
                }
                else if (Convert.ToSingle(tb_S_AddOD.Text) == 0)
                { //If AddOD (not a typo) is also zero
                    rb_S_Single.Checked = true;
                }
            }
            catch { }
