...
namespace SecondSight
{
    public partial class MainForm
    {
        ...
        private void cb_R_ReportSource_Changed(object sender, EventArgs e)
        {
            ...
            { //Source is now Current Inventory
            ...
            { //Source is now Dispensed Inventory
        ...
        private DataTable SortSummaryResults(DataTable _unsorted)
        {
            ...
            //Populate the list
            if (cb_R_GroupBy.SelectedIndex > 11)
            { //Date fields (they sort well enough on their own)
                return _unsorted;
            }
            else if (cb_R_GroupBy.SelectedIndex == 2
                  || cb_R_GroupBy.SelectedIndex == 6
                  || cb_R_GroupBy.SelectedIndex > 7)
            { //Axis and selection fields (explicitly defined sorts based on characters)
            ...
            { //Rx fields that are sorted by number (a natural sort)
