    ...
    public class SSDataBase
    {
        ...
        public DataTable ReportQuery(SSTable _table, bool _summaries, int _groupby, Dictionary<int, LinkedList<string>> _filters)
        {
            ...
            if (_summaries)
            {
                ...
                { //Type parameters - grouped by themselves (include "B" in category "M")
                ...
                { //Size parameters - grouped by themselves (include "C" in category "S")
                ...
                { //Other attribute parameters - grouped by themselves
                ...
                { //Date parameters - grouped by month
            ...
            if (_filters.Count > 0)
            {
                ...
                foreach (KeyValuePair<int, LinkedList<string>> entry in _filters)
                {
                    ...
                    if (entry.Key < 9)
                    { //Rx or date fields - use BETWEEN and pairs of values
                    ...
                    { //date fields - use BETWEEN and pairs of values and SQL DATE function
                        LinkedList<string>.Enumerator lliter = entry.Value.GetEnumerator();
                        lliter.MoveNext();
                    ...
                    { //Selection fields
                        ...
                        if (entry.Key == 11 && lliter.Current == "S")
                        { //If Size is small, include Child size in query (for legacy databases)
                            filterstring.Append(" OR ").Append(RLIMITS_FILTERSTRING[entry.Key]).Append("='C'");
                        }
                        if (entry.Key == 10 && lliter.Current == "M")
                        { //Include "B" as another way of specifying multifocal lenses (for legacy databases)
