...
namespace SecondSight
{
    public partial class MainForm
    {
        ...
        private void D_UpdateInfo(bool _isdispensing)
        {
            ...
            {  //The selected value cannot convert to integer (should never get here)
                return;
            }
        ...
        private void D_RemoveFromList(bool _isdispensing)
        {
            ...
            if (dtSelectedIndex >= 0)
            { //Negative means there's no corresponding entry in the data table
            ...
            if (tlb.Items.Count > 0)
            {  //Listbox not empty, update selected index
                if (lbSelectedIndex < tlb.Items.Count)
                { //Keep the selected index the same
            ...
            { //Listbox empty, reset all the controls in the Detailed Information groupbox
                tb_D_SphereOD.Clear();
                tb_D_CylOD.Clear();
                tb_D_AxisOD.Clear();
                tb_D_AddOD.Clear();
